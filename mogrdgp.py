...
# clients matrix
pos_C = [[6,1],[0,4],[7,6],[3,8]]#[[x1,y1],[x2,y2]...]
C = set(range(len(pos_C)))

# stations matrix
pos_E = [[2,2],[4,4],[1,6]]#[[x1,y1],[x2,y2]...]
...
batRate = [[model.add_var(var_type=INTEGER) for t in T] for u in U]
...
model.objective = minimize(xsum(VEV*vel[u][t]/V_max + on[u][t]*FEV for u in U for t in T) - 5 * finalCharge)
...
# route final time
#for u in U:
 #   model += time >= xsum(velInv[u][t] for t in T) + xsum(rechargeRate[u][e][t]*DOR for e in E for t in T)
...
if model.num_solutions:
	out.write('routes with total cost %g found: [%s,%s]' % (model.objective_value, pos_x[u][0], pos_y[u][0]))
	for u in U:
		out.write('[%s,%s]' % (pos_x[u][0], pos_y[u][0]))
		t = 1
		while True:        	
			if on[u][t] < 0.99:
				break
			out.write(' -> [%s,%s]' % (pos_x[u][t], pos_y[u][t]))
			t += 1
		out.write('\n')

		############################

		out.write('VEL: %s' % (vel[u][0]))
		t = 1
		while True:        	
			if on[u][t] < 0.99:
				break
